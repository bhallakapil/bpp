 ...
 */
{
	...
	for (i = 0; i < NCOMPASS; i++)
	{
		...
		for (j = 0; j < NCARDSPERHAND; j++)
		{
			...
			putc(card <= 2 * NCARDSPERHAND ? 'A' + card - 1 : 'a' + card - 2 * NCARDSPERHAND - 1,
				 out);
		}
	}
	fprintf(out, "\n");
...
static int theory_lsuitfreq[14] = {
	/* probability * 1000000 */
